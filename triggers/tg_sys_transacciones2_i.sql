...
GO
ALTER TRIGGER [dbo].[tg_sys_transacciones2_i] ON [dbo].[ew_sys_transacciones2] 
FOR INSERT
AS
...
DECLARE cur_c_tran2_i CURSOR LOCAL FOR 
	SELECT
		i.idtran
		, i.idestado
		, RTRIM(e.comando), RTRIM(CONVERT(VARCHAR(18),i.fechahora,3))
		, CONVERT(VARCHAR(3),i.idu)
		, i.host
	FROM
		inserted AS i
...
BEGIN
	...
	BEGIN
		...
		SELECT @comando=REPLACE(@comando, '{idestado}', RTRIM(CONVERT(VARCHAR(3),@idestado)))
