...
-- =================================================================
CREATE TRIGGER [dbo].[tg_cxp_transacciones_mov_id] ON [dbo].[ew_cxp_transacciones_mov] 
FOR INSERT
AS
...
DECLARE tg_cxp_transacciones_mov_id CURSOR FOR
	SELECT
		m.idr
		, m.idtran
		, m.idmov
...
OPEN tg_cxp_transacciones_mov_id

FETCH NEXT FROM tg_cxp_transacciones_mov_id INTO 
	@idr
	, @idtran
	, @idmov
...
BEGIN
	...
	UPDATE ew_cxp_transacciones_mov SET
		idmov = @idmov2
	WHERE 
		idr = @idr
...
CLOSE tg_cxp_transacciones_mov_id
DEALLOCATE tg_cxp_transacciones_mov_id
GO
