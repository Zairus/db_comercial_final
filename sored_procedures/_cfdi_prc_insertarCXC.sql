...
------------------------------------------------------------------
--SELECT @rfc = f.rfc FROM dbo.ew_ven_transacciones v LEFT JOIN dbo.ew_clientes_facturacion f ON f.idcliente=0 AND f.idfacturacion=0 WHERE v.idtran=@idtran
--SELECT @rfc=REPLACE(REPLACE(@rfc,' ',''),'-','')
--IF NOT EXISTS(SELECT cfd_nombre FROM dbo.ew_cfd_rfc WHERE cfd_rfc=@rfc)
--INSERT INTO dbo.ew_cfd_rfc (cfd_rfc, cfd_nombre) SELECT @rfc,f.razon_social FROM dbo.ew_cxc_transacciones v LEFT JOIN dbo.ew_clientes_facturacion f ON f.idcliente=0 AND f.idfacturacion=0 WHERE v.idtran=@idtran

SELECT @rfc=RTRIM(REPLACE(f.rfc,'-','')), @razon_social=f.razon_social FROM dbo.ew_cxc_transacciones v LEFT JOIN dbo.ew_clientes_facturacion f ON f.idcliente=v.idcliente AND f.idfacturacion=ISNULL(v.idfacturacion,0) WHERE v.idtran=@idtran
SELECT @rfc=REPLACE(REPLACE(@rfc,' ',''),'-','')
IF NOT EXISTS(SELECT cfd_nombre FROM dbo.ew_cfd_rfc WHERE cfd_rfc=@rfc)
	INSERT INTO dbo.ew_cfd_rfc (cfd_rfc, cfd_nombre) SELECT @rfc,f.razon_social FROM dbo.ew_cxc_transacciones v LEFT JOIN dbo.ew_clientes_facturacion f ON f.idcliente=v.idcliente AND f.idfacturacion=ISNULL(v.idfacturacion,0) WHERE v.idtran=@idtran
ELSE
	UPDATE dbo.ew_cfd_rfc SET cfd_nombre=@razon_social WHERE cfd_rfc=@rfc
...
--	,[cfd_Moneda]=CASE WHEN v.idmoneda=0 THEN 'MXP' ELSE bm.nombre_corto END
--	,[cfd_TipoCambio]=CASE WHEN v.idmoneda=0 THEN 1 ELSE v.tipocambio END	
FROM
	dbo.ew_cxc_transacciones v
	LEFT JOIN dbo.ew_cfd_folios f ON f.idfolio=@cfd_idfolio
	LEFT JOIN dbo.ew_clientes_facturacion cf ON cf.idcliente=v.idcliente AND cf.idfacturacion=ISNULL(v.idfacturacion,0)
...
SELECT	@concepto=ISNULL(nombre,'Descuento')
FROM	conceptos 
WHERE	idconcepto=(SELECT idconcepto FROM dbo.ew_cxc_transacciones WHERE idtran=@idtran)
SELECT @idtran2=ISNULL(idtran2,0) FROM dbo.ew_cxc_transacciones WHERE idtran=@idtran
IF @idtran2> 0
BEGIN
	...
	FROM	
		dbo.ew_cxc_transacciones m
		LEFT JOIN dbo.ew_sys_transacciones t ON t.idtran=m.idtran
		LEFT JOIN dbo.ew_cxc_transacciones m2 ON m2.idtran=m.idtran2
	WHERE
		m.idtran=@idtran
...
BEGIN
	...
	SELECT
		...
		,[importe]=m.importe
	FROM	
		dbo.ew_cxc_transacciones_mov m
--		LEFT JOIN dbo.ew_sys_transacciones t ON t.idtran=m.idtran2
		LEFT JOIN dbo.ew_cxc_transacciones t ON t.idtran=m.idtran2
	WHERE
		m.idtran=@idtran
	ORDER BY 
		m.consecutivo
