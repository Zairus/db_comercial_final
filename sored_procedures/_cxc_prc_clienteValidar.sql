...
IF @documentos_vencidos > 0 AND @autorizacion = 0 AND [dbo].[_sys_fnc_parametroActivo]('CXC_BLOQUEARVENCIDO') = 1
BEGIN
	SELECT 
		@mensaje = (
			@mensaje 
			+ 'El cliente tiene ' 
			+ LTRIM(RTRIM(STR(@documentos_vencidos))) 
			+ ' documentos vencidos.' 
			+ CHAR(13) 
			+ '** COMUNICARSE A COBRANZA, ELLOS DECIDIRÁN LA AUTORIZACIÓN **' 
			+ CHAR(13)
		)
END

IF @saldo > @credito_limite AND @autorizacion = 0 AND [dbo].[_sys_fnc_parametroActivo]('CXC_BLOQUEARLIMITE') = 1
BEGIN
	SELECT 
		@mensaje = (
			@mensaje 
			+ 'Cliente excede límite de crédito: Saldo = ' 
			+ CONVERT(VARCHAR(20), @saldo) 
			+ ', Límite = ' 
			+ CONVERT(VARCHAR(20), @credito_limite) + CHAR(13) 
			+ '** COMUNICARSE A COBRANZA, ELLOS DECIDIRÁN LA AUTORIZACIÓN **' 
			+ CHAR(13)
		)
END

IF LEN(ISNULL(@mensaje, '')) > 0
BEGIN
