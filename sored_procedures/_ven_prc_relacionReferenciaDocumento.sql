...
-- =============================================
ALTER PROCEDURE [dbo].[_ven_prc_relacionReferenciaDocumento]
	@idtran AS INT
AS
...
SELECT TOP 1
	...
	,[dias_entrega] = 0 --vt.dias_entrega
	,[t_credito] = ctran.credito
	,[credito] = CONVERT(TINYINT, vt.credito)
	...
	,[idforma] = (CASE WHEN ctran.credito = 1 THEN (SELECT TOP 1 bfa.idforma FROM ew_ban_formas_aplica AS bfa WHERE bfa.codigo = '99') ELSE 0 END)
	,[idmetodo] = (CASE WHEN ctran.credito = 1 THEN 2 ELSE 1 END)

	,[saldoAfavor] = (SELECT ISNULL(SUM(ct.saldo),0) FROM ew_cxc_transacciones AS ct  WHERE ct.cancelado = 0 AND ct.tipo = 2 AND ct.saldo > 0 AND ct.idcliente = vt.idcliente)
