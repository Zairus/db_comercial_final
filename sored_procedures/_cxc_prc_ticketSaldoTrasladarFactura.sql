...
DECLARE
	@saldo_control AS DECIMAL(18,6)

SELECT
	@saldo_control = efa4.saldo - SUM(dbo._cxc_fnc_documentoSaldoR2(ctr.idtran2, efa4.fecha))
FROM 
	ew_cxc_transacciones_rel AS ctr
...
SELECT @saldo_control = ISNULL(@saldo_control, 0)

IF @saldo_control > 0
BEGIN
	SELECT @saldo_control = @saldo_control
	SELECT @abono_idtran = NULL
	...
	SELECT
		...
		,[redondeo] = 0
		,[idu] = @idu
		,[comentario] = 'Ajuste Factura de Notas de Venta'
	FROM
		ew_cxc_transacciones AS ct
	WHERE
		ct.idtran = @idtran
	...
	SELECT
		...
		,[fecha] = @fecha
		,[tipocambio] = ct.tipocambio
		,[importe] = @saldo_control
		,[importe2] = @saldo_control
		,[impuesto1] = 0
		,[impuesto2] = 0
		,[impuesto3] = 0
		,[impuesto4] = 0
