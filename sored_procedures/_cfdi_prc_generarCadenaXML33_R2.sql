...
FROM (
	SELECT
		...
		,(CASE WHEN cc.cfd_tipoDeComprobante = 'P' THEN '0' ELSE CONVERT(VARCHAR(50), cc.cfd_subtotal) END) AS '@SubTotal'
		,(CASE WHEN cc.cfd_tipoDeComprobante = 'P' THEN '0' ELSE CONVERT(VARCHAR(50), cc.cfd_total) END) AS '@Total'
			...
			SELECT
				...
				,(
					CASE
						...
						ELSE CONVERT(VARCHAR(50), ccm.cfd_cantidad)
					END
				) AS '@Cantidad'
			...
			SELECT
				...
				,(
					SELECT
						...
						,CONVERT(DECIMAL(15,4), (cci.cfd_tasa / 100.00)) AS '@TasaOCuota'
						,SUM(cci.cfd_importe) AS '@Importe'
					FROM
						ew_cfd_comprobantes_impuesto AS cci 
						LEFT JOIN db_comercial.dbo.evoluware_cfd_sat_impuesto AS csi
